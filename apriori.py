...
def main():
    ...
    if  num_args < 4 or num_args > 6:
        print ("Expected input format: python apriori.py <index.csv> <dataset.csv> <minSup> <minConf> <name>")
        return
    elif num_args == 5 and sys.argv[2] == "--no-rules":
        dataset = csv.reader(open(sys.argv[3], "r"))
        goodsData = csv.reader(open(sys.argv[1], "r"))
        minSup  = float(sys.argv[4])
        minConf = float(sys.argv[5])
        noRules = True
        print ("Index:",sys.argv[1],"Dataset: ", sys.argv[3], " MinSup: ", minSup, " MinConf: ", minConf)
    else: 
        dataset = csv.reader(open(sys.argv[2], "r"))
        goodsData = csv.reader(open(sys.argv[1], "r"))
        minSup  = float(sys.argv[3])
        minConf = float(sys.argv[4])
        print ("Index:",sys.argv[1],"Dataset: ", sys.argv[2], " MinSup: ", minSup, " MinConf: ", minConf)
    ...
    itemsetout = open('itemset-result/'+sys.argv[5]+'-itemset.txt','w')
    rulesout = open('rules-result/'+sys.argv[5]+'-rules.txt','w')
    ...
    for k, item in frequentItemsets.items():
        for i in item:
            if k >= 2:
                ...
                print ("Itemset %d: { %s },support=%f" % (count,readable(i, goods),a.support(a.freqList[i])))
                itemsetout.write("{%s}" % (readable(i, goods))+'\n')

    print ("Jumlah itemset: %s "  %(count))
    print ("\n")
    ...
    if not noRules:
        ...
        for i, rule in enumerate(rules):
            ...
            print ("Rule %d: %s --> %s , [sup=%f, conf=%f]" % (i+1,readable(rule[0], goods), readable(rule[1], goods),rule[2],rule[3]))
            count += 1
            rulesout.write("%s --> %s" % (readable(rule[0], goods), readable(rule[1], goods))+'\n')

    print ("Jumlah Rule: %s "  %(count))
    print ("\n")
